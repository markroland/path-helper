...
class PathHelper {
  ...
  /**
   * Wrapper for Chaikin Algorithm, which handles open/closed
   * @param {array} - An array of Points
   * @param {number} - Number of recursive calls
   * @returns {array} A Path array
   */
  chaikinCurve(path, iterations = 1) {

    // Ensure number is an integer greater than zero
    iterations = Math.floor(iterations);

    // Determine if original path is open or closed
    const open = this.closedPath(path) ? false : true;

    // Perform chaikin algorithm
    let curve = this.chaikin(path, iterations);

    // Modify open and closed paths to more closely resemble
    // the original path
    if (open) {

      // Calculate the number of points to trim from the end
      // of the new curve for an open path
      const trimLength = (function(num) {
        const f = function(n) {
          if (n >= 1) {
            return 2 ** n + f(n - 1);
          } else {
      ...
      // Remove some points from the end
      curve = curve.slice(0, -trimLength);

      // Add original last point
      curve.splice(-1, 0, path[path.length - 1])

      // Add original first point
      curve.splice(0, 0, path[0])

    } else {

      // For closed paths, add first point of new curve
      curve.push(curve[0]);
    }
  ...
  /**
   * Chaikin Algorithm for curve generation
   * Invented by George Chaikin. Written by Paul Chaikin
   * Source: https://observablehq.com/@pamacha/chaikins-algorithm
   * @param {array} - An array of Points
   * @param {number} - Number of recursive calls
   * @returns {array} A Path array
  */
  chaikin(arr, num) {
    ...
    return this.chaikin(smooth, num - 1);
  }
   ...
   * @param {array} point1 - A Point array defining the start of a line segment
   * @param {array} point2 - A Point array defining the end of a line segment
   ...
   */
  lineCircleIntersect(point1, point2, circle) {

    const p1 = {x: point1[0], y: point1[1]};
    const p2 = {x: point2[0], y: point2[1]};
    const center = [circle[0][0], circle[0][1]]
    ...
    const dy = p2.y - p1.y;
    const a = dx * dx + dy * dy;
    const b = 2 * (dx * (p1.x - center[0]) + dy * (p1.y - center[1]));
    const c = (p1.x - center[0]) * (p1.x - center[0]) + (p1.y - center[1]) * (p1.y - center[1]) - radius * radius;
    ...
    if (discriminant === 0) {  // Tangent
      if (t1 >= 0 && t1 <= 1) {
        intersections.push([p1.x + t1 * dx, p1.y + t1 * dy]);
      }
    } else { // Two intersection points (or secant line)
      if (t1 >= 0 && t1 <= 1) {
        intersections.push([p1.x + t1 * dx, p1.y + t1 * dy]);
      }
      if (t2 >= 0 && t2 <= 1) {
        intersections.push([p1.x + t2 * dx, p1.y + t2 * dy]);
      }
    }
   ...
   * @param {number} [max_offset=0] - The maximum stroke offset
   * @param {number} [period=0] - The distance between envelope points. Minimum value should be close to stroke.
   * Maximum value depends on path. Lower value creates more variation.
   ...
   * @returns {array} A Path array
   **/
  varyPath(path, stroke_width, max_offset=0, period=0, smoothing_window = 7, clamp_endpoints = true, simplify_distance = null) {
    ...
    if ( !envelope.length ) {
      ...
      // Require the smoothing window to be 3 or more
  ...
  splitPathByDistance(path, distance) {
    ...
    // This is important to make a deep copy otherwise the
    ...
    for (let i = 1; i < path.length; i++) {
      ...
      // Get distance of current segment
      const segment_distance = Math.hypot(path[i][0] - path[i-1][0], path[i][1] - path[i-1][1]);
      ...
      if (total_distance + segment_distance < distance) {
        ...
        // Remove point from remaining path
        ...
        // Remove the point from the remaining path and add the new starting point
        remaining_path.shift();
        // remaining_path = path.slice(i);
        remaining_path.unshift(new_segment[1]);
        break;
      }
    }
  ...
  /**
   * Break apart a single path into segments using its
   * existing points.
   * @param {array} path - A Path array
   * @returns {array} An array of Path arrays
   */
